...
import { HashPath, MerkleTree, Sha256Hasher } from 'src'
...
describe('testMerkleTree', () => {
  ...
  test('should have correct empty tree root for depth 32', async () => {
  ...
  test('should have correct root', async () => {
    const db = levelup(memdown())
    ...
    expect(await tree.getHashPath2(0)).toEqual(expected)
    expect(await tree.getHashPath2(1)).toEqual(expected)
    ...
    expect(await tree.getHashPath2(2)).toEqual(expected)
    expect(await tree.getHashPath2(3)).toEqual(expected)

    expect(root).toEqual(Buffer.from('e645e6b5445483a358c4d15c1923c616a0e6884906b05c196d341ece93b2de42', 'hex'))
  })

  test('should be able to restore from previous data', async () => {
    ...
    for (let i = 0; i < 128; ++i)
      expect(await tree.getHashPath2(i)).toEqual(await tree2.getHashPath2(i))
  }, 10000)

  test('should have correct results inserting 1024 values into 32 depth tree.', async () => {
    const db = levelup(memdown())
    const tree = await MerkleTree.new(db, 'test', 32)
    ...
    const hashPath = await tree.getHashPath2(100)
